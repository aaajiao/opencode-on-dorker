...
ARG PIP_PANDAS=3.0.1
ARG PIP_NUMPY=2.4.2
ARG PIP_MATPLOTLIB=3.10.8
ARG PIP_BEAUTIFULSOUP4=4.14.3
ARG PIP_PILLOW=12.1.1
ARG PIP_NOTEBOOKLM_PY=0.3.3
ARG OPENCODE_AI_VERSION=1.2.23
...
# -------------------------------------------------------
# 第十四步：Entrypoint 脚本 - 处理环境变量、GitHub 认证、浏览器修复
# -------------------------------------------------------
RUN echo '#!/bin/bash\n\
\n\
# 调试函数\n\
